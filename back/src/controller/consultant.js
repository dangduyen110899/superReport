...
consultant.detailList = async (req, res) => {
  const year = req.query.year;
  const semester = req.query.semester;
  let response, countx
  const page = Number(req.query.page)
...
consultant.creates = async (req, res) => {
    ...
    readXlsxFile(filePath).then(rows => {
      ...
      const fetchApi = async () => {
        for (let i = 0; i < rows.length; i++) {
            const tempTeacher = rows[i][4].split('\n');
            ...
            if (tempTeacher.length===1) {
              const res1 = await Lecturer.findAll({where: {name: rows[i][4].trim()}})
              if (res1.length<1) {
                res.json({message: `Lecturer ${rows[i][4]} not exit.`});
              }
              ...
              let consultant = {
                ...
                lecturerName: rows[i][4].trim(),
                studentId	: res2[0].dataValues.id,
                year: year,
                semester: semester
              }
              consultant.hour = getHourItem(consultant, null , 'cvht',1,1)
              theses.push(consultant);
            } else {
              ...
              await Promise.all(
                ...
                for (let j = 0; j < res3.length; j++) {
                  ...
                  consultant1.hour = getHourItem(consultant1, null , 'cvht', res3.length, j+1)
                  theses.push(consultant1);
                }
              })
            }
        }
      ...
      fetchApi().then( async () => {
        try {
          await Consultant.bulkCreate(theses).then(() =>{
            // res.json(theses);
           })
        } catch (error) {
          console.log(error)
        ...
        for (let i = 0; i < lecturerIdKltn.length; i++) {
          await report.updateHour(year, semester, 'cvht', lecturerIdKltn[i])
        }
...
consultant.checkYear = async (req, res) => {
  const year = req.body.year;
  const semester = req.body.semester;
  try {
    await Consultant.findAll({where: {
      [Op.and]: [
        { year: year },
        { semester: Number(semester) }
      ]}
